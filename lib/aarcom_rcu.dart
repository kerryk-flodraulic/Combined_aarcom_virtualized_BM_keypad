...
class AARCommRCU extends StatefulWidget {
  const AARCommRCU({super.key});
  @override
  State<AARCommRCU> createState() => _AARCommRCUState();
}

// Internal state class for AARCommRCU
class _AARCommRCUState extends State<AARCommRCU> {
  final ScrollController _scrollController = ScrollController();

  String _currentTestLabel = ''; // Displays "Testing: <control>"

bool _showCanFrame = true; // Toggle visibility of CAN frame log
...
//Pratik mentioned may not be necessary - REMOVE WHEN TESTING
    ...
    /* CanBluetooth.instance.startScan(
      onDeviceFound: (device, advertisementData) async {
        ...
        if (name == 'fd000000000i0' || id == 'fd000000000i0') {
  ...
  // Starts a timer that simulates transmitting a CAN frame every 1 second  Only runs if the live feed is active and not paused , was (milliseconds: 250)
  void _startLiveFeed() {
              ...
              Center(
                child: Wrap(
                  ...
                  children: controls.map((control) {
                    final isActive =
                        states[control] ?? false; // Get control state
                    ...
                    return GestureDetector(
                      ...
                      onTap: () {
                        setState(() {
                          ...
                          final key =
                              '${frame[3]}|${frame[6]}'; // Data|Contents key
                      ...
                      child: AnimatedContainer(
                        duration: const Duration(
                            milliseconds: 250), // Animate state changes
                        padding: const EdgeInsets.symmetric(
                            horizontal: 12, vertical: 8),
                        decoration: BoxDecoration(
                          color: isActive
                              ? Colors.tealAccent
                                  .withOpacity(0.2) // Active background
        ...
        children: [
          ...
          Expanded(
            child: ListView.builder(
              controller:
                  _scrollController, // Scroll controller for auto-scroll
            ...
            children: [
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    if (_currentTestLabel.isNotEmpty)
                      Padding(
                    ...
                    Wrap(
                      ...
                      children: [
                        ...
                        //NEW Auto Test Feature
                        // This button simulates pressing each control one at a time.
                        // It logs the generated CAN frame and sends it over Bluetooth.
                        ...
                        ElevatedButton.icon(
                          ...
                          onPressed: () async {
                            // Check that a Bluetooth device is connected
                            if (CanBluetooth
                                .instance.connectedDevices.isEmpty) {
                              // If not connected, show a message and exit
                            ...
                            // Get a list of all available control button names
                            final allKeys = states.keys
                                .toList(); // e.g. ['Water Pump On', 'Boom Up', ...]

                            //  Loop through each button one by one
                            for (String control in allKeys) {
                              setState(() {
                                _currentTestLabel = 'Testing: $control';
                              });

                              setState(() {
                                // Reset all button states (turn all OFF)
                                states.updateAll((_, __) => false);

                                // Activate (turn ON) only the current control
                                states[control] = true;

                                //Apply mutual exclusivity rules to prevent conflicts
                                final exclusivityRules = {
                                  'Water Pump On': ['Water Pump Off'],
                                  'Water Pump Off': ['Water Pump On'],
                                  'Water Pressure Increase': [
                                ...
                                // If the current control has any conflicting controls, turn THE OTHER off e.g Water Pump on and Water Pump off cannot be on at the same time
                                ...
                                // Generate the byte array representing the current button state
                                final bytes = getByteValues();

                                //  Create a CAN frame with timestamp and button info
                                final frame = createCanFrame(
                                  bytes,
                                  DateTime.now()
                                      .difference(firstButtonPressTime!),
                                ...
                                // Add the frame to both live and fixed logs
                                final key =
                                    '${frame[3]}|${frame[6]}'; // Unique key = Data | Contents
                                fixedCanHistoryMap[key] = frame;
                                canHistory.add(frame);
                            ...
                            setState(() {
                              states
                                  .updateAll((_, __) => false); // Reset buttons
                              _currentTestLabel = ''; // Clear test label
                            });

                            //  After the loop, reset all buttons to OFF
                            setState(() {
                              states.updateAll((_, __) => false);
                            });
                          },
    ...
    child: Column(
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: const [
            Icon(Icons.directions_car, color: Colors.tealAccent),
            SizedBox(width: 8),
            Text(
              'CAN Frame Logs',
              style: TextStyle(
        ...
        ElevatedButton.icon(
          ...
          icon: Icon(_showCanFrame ? Icons.visibility_off : Icons.visibility),
          label: Text(_showCanFrame ? 'Hide CAN Frame' : 'Show CAN Frame'),
          style: ElevatedButton.styleFrom(
